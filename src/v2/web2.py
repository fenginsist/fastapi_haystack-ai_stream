...
class ChatGLM(OpenAIGenerator):
    def __init__(
        ...
        # self.haystack_streaming_callback = haystack_streaming_callback
        super().__init__(
            api_key=api_key,
            model=model,
        ...
        pass
    # def set_haystack_streaming_callback(self, callback):
    #     """设置动态回调函数"""
    #     super().streaming_callback = callback
    #     pass
...
# 初始化 OpenAI Generator 节点 由上面的 ChatGLM 代替
...
openai_generator = ChatGLM(
    system_prompt="你作为高中语文老师，按照要求回答问题即可",
    haystack_streaming_callback = haystack_stream
)
...
'''
运行命令: nohup uvicorn web2:app --host 0.0.0.0 --port 8001 --reload > nohup2.out 2>&1 &

基础版本2: 实现了基本的流式输出和非流失输出

和上一个版本相比：对大模型组件进行了自定义继承和改进。

问题: 异步队列是全局的，当同一时刻发送多请求，会导致回答混乱。
'''